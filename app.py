...
# ✅ Check .env safety
if not all(CRM_DB_CONN.values()):
    raise ValueError("❌ Missing DB connection env vars!")

# ✅ Input schema
class QueryZohoSQLArgs(BaseModel):
    sql: str = Field(..., description="The full SELECT SQL to run. Must be generated by the LLM only.")
...
def query_zoho_leads_sql(sql: str, limit: int = 10) -> str:
    ...
    if not rows:
        return "✅ Query ran but no rows matched."
